...
def info():
    try:
        ...
        price_table = soup.select('div.section.trade_compare')[
            0]  # div태그의 클래스 값이 section.trade_compare인 태그를 price_table에 넣어줌
        price = [item.get_text().strip() for item in price_table.select('td')][
            0]  # price_table의 첫번째 td태그에서 문자열을 택해 앞뒤 공백을 제거 후 현재가를 가져옴
        price_table_tr = price_table.select('tr')[3]  # price_table의 3번째 tr태그
        fluctuation = [item.get_text().strip() for item in price_table_tr.select('td')][
            0]  # price_table의 3번째 tr태그에서 0번째 td태그안의 등락률을 가져옴
        ...
        same_type = [item.get_text().strip() for item in same_types.select('th')][
                    2:]  # same_types의 두번째 th인 동일업종부터 same_type에 넣어줌
        ...
        PER_TABLE = [item.get_text().strip() for item in per_table.select('em')][
                    0:7]  # PER_TABLE리스트에 첫 em태그부터 6번째 em태그까지의 PER EPS 추정PER 추정EPS PBR BPS 배당수익률 을 넣어줌
